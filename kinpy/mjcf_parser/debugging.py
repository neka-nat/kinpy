...
flags.DEFINE_string("pymjcf_debug_full_dump_dir", "", "Path to dump full debug info when Mujoco error is encountered.")

StackTraceEntry = collections.namedtuple("StackTraceEntry", ("filename", "line_number", "function_name", "text"))

ElementDebugInfo = collections.namedtuple("ElementDebugInfo", ("element", "init_stack", "attribute_stacks"))

MODULE_PATH = os.path.dirname(sys.modules[__name__].__file__)
DEBUG_METADATA_PREFIX = "pymjcfdebug"

_DEBUG_METADATA_TAG_PREFIX = "<!--" + DEBUG_METADATA_PREFIX
_DEBUG_METADATA_SEARCH_PATTERN = re.compile(r"<!--{}:(\d+)-->".format(DEBUG_METADATA_PREFIX))
...
def _get_actual_current_stack_trace():
    ...
    for raw_stack_item in raw_stack:
        ...
        if stack_item.filename.startswith(MODULE_PATH) and not stack_item.filename.endswith("_test.py"):
            break
        else:
...
class DebugContext(object):
    ...
    def process_and_raise_last_exception(self):
        ...
        if debug_mode():
            ...
            if xml_line:
                message_lines.extend([stripped_xml_line, self._generate_debug_message_from_xml_line(xml_line)])
        else:
        ...
        def dump_stack(header, stack, f):
            ...
            for stack_entry in stack:
                f.write(
                    indent
                    + "`{}` at {}:{}\n".format(
                        stack_entry.function_name, stack_entry.filename, stack_entry.line_number
                    )
                )
            ...
            with open(os.path.join(dump_dir, str(elem_id) + ".dump"), "w") as f:
                ...
                for attrib_name, stack in six.iteritems(debug_info.attribute_stacks):
                    ...
                    if stack[-1] == debug_info.init_stack[-1]:
                        ...
                        if attrib_value is not None:
                            dump_stack('Attribute {}="{}"'.format(attrib_name, attrib_value), stack, f)
                        else:
                            dump_stack("Attribute {} was CLEARED".format(attrib_name), stack, f)
    ...
    def _generate_debug_message_from_element_id(self, elem_id):
        ...
        for attrib_name, stack in six.iteritems(debug_info.attribute_stacks):
            ...
            if stack[-1] == debug_info.init_stack[-1]:
                if attrib_value is not None:
                    out.append('  * {}="{}" was set when the element was created'.format(attrib_name, attrib_value))
            else:
                if attrib_value is not None:
                    out.append(
                    ...
                    out.append(
                        "  * {} was CLEARED by `{}` at {}:{}".format(
                            attrib_name, stack[-1].function_name, stack[-1].filename, stack[-1].line_number
                        )
                    )
